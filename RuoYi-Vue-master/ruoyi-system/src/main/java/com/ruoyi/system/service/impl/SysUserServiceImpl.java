...
public class SysUserServiceImpl implements ISysUserService {
    ...
    @Override
    @Transactional
    public int insertGroupLeader(SysUser user,long timestamp) {
        // 新增用户信息
        user.setUserId(timestamp);
        int rows = userMapper.insertGroupLeader(user);
        // 新增用户岗位关联
        insertUserPost(user);
        // 新增用户与角色管理
        insertUserRole(user);
...
//    public ArrayList<CustomerServiceDetailTemp> addBatch(SysUser user, Long maxLoad) {
//        if (!userService.checkUserNameUnique(user)) {
//            throw new ServiceException("该账户前缀已存在");
//        }
...
//        ArrayList<CustomerServiceDetailTemp> CustomerServiceDetailList = new ArrayList<>();
//        if (user.getIsManage().equals(1)) {
//            //此处生成一个管理号
//            SysUser adminUser = new SysUser();
//            BeanUtils.copyBeanProp(adminUser, user);
//            String adminName = user.getUserName() + "admin";
...
//            CustomerServiceDetailTemp customerServiceDetailTemp =
//                    new CustomerServiceDetailTemp(user.getProjectId(), maxLoad, userId, -1L);
//            customerServiceDetailTemp.setUserName(adminUser.getUserName());
//            userMapper.insertCustomerServiceDetail(customerServiceDetailTemp);
...
//        ArrayList<CustomerServiceDetailTemp> CustomerServiceDetail = new ArrayList<>();
//
//        for (int i = 0; i < genNum; i++) {
//
//            int nameNumTemp = i + 1;
//            String nameNum = String.valueOf(nameNumTemp);
//            // 根据数字的位数进行补零操作
...
//            CustomerServiceDetailTemp customerServiceDetailTemp =
//                    new CustomerServiceDetailTemp(user.getProjectId(), maxLoad, timestamp, -2L);
...
//        userMapper.insertCustomerServiceDetailBatch(CustomerServiceDetail);
//
...
//    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void insertUserBatch(ArrayList<SysUser> sysUserList) {
        ...
        userMapper.insertUserBatch(sysUserList);
    }

    @Override
    public ArrayList<SysUser> selectUserByUserId(HashSet<Long> userList) {
        return userMapper.selectUserByIdList(userList);
    }
}
